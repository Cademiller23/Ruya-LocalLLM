...
2. After successful login, Run the following command to create a deployment using the Deployment Manager CLI:
  ...
  gcloud deployment-manager deployments create ruya-llm-deployment --config gcp/deployment/gcp_deploy_anything_llm.yaml
...
```
gcloud compute instances get-serial-port-output ruya-llm-instance 
```

ssh into the instance

```
gcloud compute ssh ruya-llm-instance 
```

Delete the deployment
```
gcloud deployment-manager deployments delete ruya-llm-deployment 
```
