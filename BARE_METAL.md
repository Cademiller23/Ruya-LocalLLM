# Run RuyaLLM in production without Docker
...
Here you can find the scripts and known working process to run RuyaLLM outside of a Docker container.
...
`git clone git@github.com:Mintplex-Labs/ruya-llm.git`

2. `cd ruya-llm` and run `yarn setup`. This will install all dependencies to run in production as well as debug the application.
...
RuyaLLM is comprised of three main sections. The `frontend`, `server`, and `collector`. When running in production you will be running `server` and `collector` on two different processes, with a build step for compilation of the frontend.
...
RuyaLLM should now be running on `http://localhost:3001`!

## Updating RuyaLLM

To update RuyaLLM with future updates you can `git pull origin master` to pull in the latest code and then repeat steps 2 - 5 to deploy with all changes fully.
...
_note_ You should `pkill node` before running an update so that you are not running multiple RuyaLLM processes on the same instance as this can cause conflicts.
...
cd $HOME/ruya-llm/frontend && yarn && yarn build && cd $HOME/ruya-llm
...
cd $HOME/ruya-llm/collector && yarn

echo "Installing server dependencies & running migrations"
cd $HOME/ruya-llm/server && yarn
cd $HOME/ruya-llm/server && npx prisma migrate deploy --schema=./prisma/schema.prisma
cd $HOME/ruya-llm/server && npx prisma generate

echo "Booting up services."
truncate -s 0 /logs/server.log # Or any other log file location.
truncate -s 0 /logs/collector.log

cd $HOME/ruya-llm/server
(NODE_ENV=production node index.js) &> /logs/server.log &

cd $HOME/ruya-llm/collector
